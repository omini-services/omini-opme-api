...
public interface IRespositoryDocumentEntity<TEntity> : IDisposable where TEntity : DocumentEntity
{
    ...
    IQueryable<TEntity> OrderBy(IQueryable<TEntity> query, string? orderByField = null, SortDirection sortDirection = SortDirection.Asc, CancellationToken cancellationToken = default);
    Task<TEntity?> GetByNumber(long number, CancellationToken cancellationToken = default);
    Task<TEntity?> GetById(Guid id, CancellationToken cancellationToken = default);
    Task<PagedResult<TEntity>> GetAll(int currentPage = 1, int pageSize = 100, string? orderByField = null, SortDirection sortDirection = SortDirection.Asc, string? queryValue = null, CancellationToken cancellationToken = default);
    ...
    IQueryable<TEntity> Filter(IQueryable<TEntity> query, string? queryValue = null);
}